...
watch(
	() => route.query,
	async () => {
		const { data, error, refresh } = await useFetch('/api/producerProfile/getProfiles', {
			query: {
				category: category.value,
				opportunity: LFopportunity.value,
				updated: updated.value
...
<template>
	<div style="position: relative" class="cards">
		<h2 class="find-producer">Find <span class="green-span">Producers</span></h2>
		...
		<div v-else-if="loading" class="lds-dual-ring" style="padding-left: 20rem; top: 20%"></div>
	</div>
</template>
